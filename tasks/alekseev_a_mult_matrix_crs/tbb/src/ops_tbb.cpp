...
AlekseevAMultMatrixCRSTBB::AlekseevAMultMatrixCRSTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool AlekseevAMultMatrixCRSTBB::ValidationImpl() {
  const auto &a = std::get<0>(GetInput());
  const auto &b = std::get<1>(GetInput());
  return a.cols == b.rows && !a.row_ptr.empty() && !b.row_ptr.empty();
}

bool AlekseevAMultMatrixCRSTBB::PreProcessingImpl() {
  GetOutput() = {};
  return true;
}

void AlekseevAMultMatrixCRSTBB::ProcessRow(std::size_t i, const CRSMatrix &a, const CRSMatrix &b,
...
bool AlekseevAMultMatrixCRSTBB::RunImpl() {
  const auto &a = std::get<0>(GetInput());
  const auto &b = std::get<1>(GetInput());
  auto &c = GetOutput();
  ...
  tbb::parallel_for(tbb::blocked_range<std::size_t>(0, a.rows), [&](const tbb::blocked_range<std::size_t> &range) {
...
bool AlekseevAMultMatrixCRSTBB::PostProcessingImpl() {
  return true;
}
