...
void KhruevARadixSortingIntBatherMergeTBB::CompareExchange(std::vector<int> &a, size_t i, size_t j) {
  if (a[i] > a[j]) {
    std::swap(a[i], a[j]);
  }
}

void KhruevARadixSortingIntBatherMergeTBB::RadixSort(std::vector<int> &arr) {
  const int bits = 8;
...
void KhruevARadixSortingIntBatherMergeTBB::OddEvenMerge(std::vector<int> &a, size_t n) {
  if (n < 2) {
    return;
  }
...
KhruevARadixSortingIntBatherMergeTBB::KhruevARadixSortingIntBatherMergeTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool KhruevARadixSortingIntBatherMergeTBB::ValidationImpl() {
  return !GetInput().empty();
}

bool KhruevARadixSortingIntBatherMergeTBB::PreProcessingImpl() {
  GetOutput().resize(GetInput().size());
  return true;
}

bool KhruevARadixSortingIntBatherMergeTBB::RunImpl() {
  ...
  tbb::parallel_invoke([&]() { RadixSort(left); }, [&]() { RadixSort(right); });
...
bool KhruevARadixSortingIntBatherMergeTBB::PostProcessingImpl() {
