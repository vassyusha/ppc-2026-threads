...
KondakovVShellSortSTL::KondakovVShellSortSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = in;
}

bool KondakovVShellSortSTL::ValidationImpl() {
  return !GetInput().empty();
}

bool KondakovVShellSortSTL::PreProcessingImpl() {
  GetOutput() = GetInput();
  return true;
}

bool KondakovVShellSortSTL::RunImpl() {
  ...
  for (size_t part = 0; part < parts; ++part) {
    ...
    threads.emplace_back(ShellSort, std::ref(segments[part]));
  }
...
bool KondakovVShellSortSTL::PostProcessingImpl() {
