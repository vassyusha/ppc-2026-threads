...
LazarevaATestTaskTBB::LazarevaATestTaskTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = {};
}

bool LazarevaATestTaskTBB::ValidationImpl() {
...
bool LazarevaATestTaskTBB::PreProcessingImpl() {
  n_ = GetInput().n;
...
bool LazarevaATestTaskTBB::RunImpl() {
  result_ = Strassen(a_, b_, padded_n_);
  return true;
}

bool LazarevaATestTaskTBB::PostProcessingImpl() {
  GetOutput() = UnpadMatrix(result_, padded_n_, n_);
  return true;
}

int LazarevaATestTaskTBB::NextPowerOfTwo(int n) {
  if (n <= 0) {
    return 1;
  }
  int p = 1;
...
void LazarevaATestTaskTBB::Split(const std::vector<double> &parent, int n, std::vector<double> &a11,
...
std::vector<double> LazarevaATestTaskTBB::NaiveMult(const std::vector<double> &a, const std::vector<double> &b, int n) {
  const auto size = static_cast<size_t>(n) * static_cast<size_t>(n);
...
std::vector<double> LazarevaATestTaskTBB::Strassen(const std::vector<double> &root_a, const std::vector<double> &root_b,
                                                   int root_n) {
  if (root_n <= 64) {
