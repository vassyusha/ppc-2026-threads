...
class MatmulDoubleTBBPerfTest : public ppc::util::BaseRunPerfTests<InType, OutType> {
...
TEST_P(MatmulDoubleTBBPerfTest, RunPerfModes) {
...
const auto kAllPerfTasks =
    ppc::util::MakeAllPerfTasks<InType, MatmulDoubleTBBTask>(PPC_SETTINGS_makoveeva_matmul_double_tbb);

const auto kGtestValues = ppc::util::TupleToGTestValues(kAllPerfTasks);

const auto kPerfTestName = MatmulDoubleTBBPerfTest::CustomPerfTestName;

INSTANTIATE_TEST_SUITE_P(RunModeTests, MatmulDoubleTBBPerfTest, kGtestValues, kPerfTestName);
