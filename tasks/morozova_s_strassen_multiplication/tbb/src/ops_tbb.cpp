...
Matrix AddMatrixImpl(const Matrix &a, const Matrix &b) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int>(0, n, 0, n), [&a, &b, &result](const tbb::blocked_range2d<int> &r) {
...
Matrix MultiplyStandardImpl(const Matrix &a, const Matrix &b) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int>(0, n, 0, n), [&a, &b, &result, n](const tbb::blocked_range2d<int> &r) {
    for (int i = r.rows().begin(); i < r.rows().end(); ++i) {
      for (int j = r.cols().begin(); j < r.cols().end(); ++j) {
        double sum = 0.0;
        for (int k = 0; k < n; ++k) {
...
void SplitMatrixImpl(const Matrix &m, Matrix &m11, Matrix &m12, Matrix &m21, Matrix &m22) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int>(0, half, 0, half),
                    [&m, &m11, &m12, &m21, &m22, half](const tbb::blocked_range2d<int> &r) {
    for (int i = r.rows().begin(); i < r.rows().end(); ++i) {
      for (int j = r.cols().begin(); j < r.cols().end(); ++j) {
        m11(i, j) = m(i, j);
        m12(i, j) = m(i, j + half);
...
Matrix MergeMatricesImpl(const Matrix &m11, const Matrix &m12, const Matrix &m21, const Matrix &m22) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int>(0, half, 0, half),
                    [&m11, &m12, &m21, &m22, &result, half](const tbb::blocked_range2d<int> &r) {
    for (int i = r.rows().begin(); i < r.rows().end(); ++i) {
      for (int j = r.cols().begin(); j < r.cols().end(); ++j) {
        result(i, j) = m11(i, j);
...
MorozovaSStrassenMultiplicationTBB::MorozovaSStrassenMultiplicationTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType();
}

bool MorozovaSStrassenMultiplicationTBB::ValidationImpl() {
  return true;
}
...
bool MorozovaSStrassenMultiplicationTBB::PostProcessingImpl() {
...
Matrix MorozovaSStrassenMultiplicationTBB::AddMatrix(const Matrix &a, const Matrix &b) {
  return AddMatrixImpl(a, b);
}

Matrix MorozovaSStrassenMultiplicationTBB::SubtractMatrix(const Matrix &a, const Matrix &b) {
  return SubtractMatrixImpl(a, b);
}

Matrix MorozovaSStrassenMultiplicationTBB::MultiplyStandard(const Matrix &a, const Matrix &b) {
  return MultiplyStandardImpl(a, b);
}

void MorozovaSStrassenMultiplicationTBB::SplitMatrix(const Matrix &m, Matrix &m11, Matrix &m12, Matrix &m21,
                                                     Matrix &m22) {
  SplitMatrixImpl(m, m11, m12, m21, m22);
}

Matrix MorozovaSStrassenMultiplicationTBB::MergeMatrices(const Matrix &m11, const Matrix &m12, const Matrix &m21,
                                                         const Matrix &m22) {
  return MergeMatricesImpl(m11, m12, m21, m22);
}

Matrix MorozovaSStrassenMultiplicationTBB::MultiplyStrassen(const Matrix &a, const Matrix &b, int leaf_size) {
  return MultiplyStrassenIterative(a, b, leaf_size);
}

Matrix MorozovaSStrassenMultiplicationTBB::MultiplyStandardParallel(const Matrix &a, const Matrix &b) {
  return MultiplyStandardParallelImpl(a, b);
}
