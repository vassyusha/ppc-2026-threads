...
SpichekDRadixSortSTL::SpichekDRadixSortSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool SpichekDRadixSortSTL::ValidationImpl() {
  return !GetInput().empty();
}

bool SpichekDRadixSortSTL::PreProcessingImpl() {
  GetOutput() = GetInput();
  return true;
}

bool SpichekDRadixSortSTL::RunImpl() {
  if (GetOutput().empty()) {
    return true;
  }
...
bool SpichekDRadixSortSTL::PostProcessingImpl() {
  return std::ranges::is_sorted(GetOutput());
}

void SpichekDRadixSortSTL::RadixSort(std::vector<int> &data) {
  if (data.empty()) {
    return;
  }
